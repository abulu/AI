...
class Ranking(object):
    def __init__(
        ...
        self.input_left = tf.placeholder(
            tf.int32, [None, max_len_left], name="input_left")
        self.input_right = tf.placeholder(
            tf.int32, [None, max_len_right], name="input_right")
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            W = tf.Variable(
                ...
                name="W")
            self.embedded_chars_left = tf.expand_dims(
                tf.nn.embedding_lookup(W, self.input_left), -1)
            self.embedded_chars_right = tf.expand_dims(
                tf.nn.embedding_lookup(W, self.input_right), -1)
        ...
        self.h_pool_left = tf.reshape(
            tf.concat(pooled_outputs_left, 3), [-1, num_filters_total], name='h_pool_left')
        self.h_pool_right = tf.reshape(tf.concat(
            pooled_outputs_right, 3), [-1, num_filters_total], name='h_pool_right')
        ...
        with tf.name_scope("similarity"):
            ...
            self.sims = tf.reduce_sum(tf.multiply(
                self.transform_left, self.h_pool_right), 1, keepdims=True)
            print(self.sims)
        ...
        self.new_input = tf.concat(
            [self.h_pool_left, self.sims, self.h_pool_right], 1, name='new_input')
        ...
        with tf.name_scope("hidden"):
            ...
            self.hidden_output = tf.nn.relu(tf.nn.xw_plus_b(
                self.new_input, W, b, name="hidden_output"))

        # Add dropout
        with tf.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(
                self.hidden_output, self.dropout_keep_prob, name="hidden_output_drop")
            print(self.h_drop)
