...
tf.flags.DEFINE_integer("embedding_dim", 64,
                        "Dimensionality of character embedding (default: 64)")
tf.flags.DEFINE_string("filter_sizes", "2,3",
                       "Comma-separated filter sizes (default: '2,3')")
...
tf.flags.DEFINE_integer("max_len_right", 10,
                        "max document length of right input")
tf.flags.DEFINE_integer("most_words", 300000,
                        "Most number of words in vocab (default: 300000)")
# Training parameters
...
tf.flags.DEFINE_float(
    "eval_split", 0.1, "Use how much data for evaluating (default: 0.1)")
...
def build_vocab(sentences):
    ...
    vocabulary_inv = [x[0]
                      for x in word_counts.most_common(FLAGS.most_words-1)]
...
def build_input_data(data_left, data_right, label, vocab):
    ...
    out_left = np.array([[vocab[word] if word in vocabset else vocab['<UNK/>']
                          for word in sentence] for sentence in data_left])
    out_right = np.array([[vocab[word] if word in vocabset else vocab['<UNK/>']
                           for word in sentence] for sentence in data_right])
...
def load_data(filepath, vocab_tuple=None):
    data = list(
        set(open(filepath, 'r', encoding='utf-8', errors='ignore').readlines()))
    ...
    data_left, data_right, data_label = build_input_data(
        data_left, data_right, data_label, vocab)
    '''
    for i in xrange(10):
...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            out_dir = os.path.abspath(os.path.join(
                FLAGS.train_dir, "runs", timestamp))
    ...
    for batch in batches:
        ...
        if current_step % FLAGS.evaluate_every == 0:
            ...
            time_str = datetime.datetime.now().isoformat()
            print(
                "{}: dev-aver, loss {:g}, acc {:g}".format(time_str, loss, accuracy))
